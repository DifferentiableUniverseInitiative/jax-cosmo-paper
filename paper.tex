...
%
In the following sections we review some statistical methods used to make the cosmological parameter inference with \jaxcosmo. We have used the \textit{Stochastic Variational Inference} (aka SVI), a variant of Hamiltonian Monte Carlo knows as \textit{No-U-Turn} (aka NUTS), and finally NUTS after a \textit{Neural Transport} SVI. All  the methods have been implemented using the \numpyro Probabilistic Programming Language (PPL). 
%
...
\begin{figure}
\includegraphics[width=\columnwidth]{figures/Cobaya-NUTS-SVI.pdf}
\caption{Constraints (30\%, 68\%, 90\%) on 4 of the 21 parameters of a simulated DES-Y1 3x2pt likelihood, using the \texttt{Cobaya} Metropolis-Hastings sampler (full curve in blue), the  NUTS sampling described in section \ref{sec-NUTS} (dashed curve in red), and a SVI MVN optimisation followed by a Naural Transport parametrisation to run a NUTS sampling (dashed-dotted curve in green).}
